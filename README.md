...
- **▶️ Live demo (GitHub Pages):** **https://furqunali.github.io/lonestar-video-intelligence/**
  — the sample director report, rendered as a real web page (synthetic data).
- **Sample dashboard source:** [`docs/sample_dashboard.html`](docs/sample_dashboard.html)
  — risk-ranked incident table, KPI tiles, illustrative evidence cards, and the
  chat widget. (GitHub shows HTML files as source; use the live link above to *see* it.)
...
```mermaid
flowchart LR
  A[FreeCam MP4 exports] --> B[Ingest: decode and sample]
  B --> C[Camera health]
  C --> D[Detect and Track]
  D --> E[Zones]
  E --> F[Identity: ArUco, gated]
  F --> G[(Event store)]
  G --> H[POS-exception risk scoring]
  H --> I[Incident report / dashboard]
  I --> J{Human review}
  I --> K[Chat assistant - Stella]
```

*Pipeline: MP4 exports → ingest → camera health → YOLOv8n detect + ByteTrack →
zones → ArUco identity (gated) → event store → risk scoring → reviewed report + chat.*
